...
export async function generateMetadata({
    ...
    if (resolved.kind !== "resolved") {
        const canonical = `${siteUrl}/diesel-price/${encodeURIComponent(params.state)}/${encodeURIComponent(params.city)}`;
        return {
    ...
    const canonical = buildCanonicalCityPage("diesel-price", snap.state_code, cityName);
...
export default async function DieselPriceCityCanonicalPage({
    params,
}: {
    params: { state: string; city: string };
}) {
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                        {resolved.candidates.map((c) => (
                            <Link
                                ...
                                href={`/diesel-price/${encodeURIComponent(stateLower)}/${encodeURIComponent(
    ...
    const canonical = buildCanonicalCityPage("diesel-price", snap.state_code, cityName);
    const siteUrl = getSiteUrl();
    const stateCanonical = buildCanonicalStatePage("diesel-price", snap.state_code);
    ...
    const bread = buildBreadcrumbLd([
        ...
        { name: "Diesel price", item: `${siteUrl}/diesel-price` },
