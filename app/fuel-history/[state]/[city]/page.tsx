...
export async function generateMetadata({
    ...
    if (resolved.kind !== "resolved") {
        const canonical = `${siteUrl}/fuel-history/${encodeURIComponent(params.state)}/${encodeURIComponent(params.city)}`;
        return {
    ...
    const canonical = buildCanonicalCityPage("fuel-history", snap.state_code, cityName);
...
export default async function FuelHistoryCityCanonicalPage({
    params,
}: {
    params: { state: string; city: string };
}) {
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                        {resolved.candidates.map((c) => (
                            <Link
                                ...
                                href={`/fuel-history/${encodeURIComponent(stateLower)}/${encodeURIComponent(
    ...
    const siteUrl = getSiteUrl();
    const canonical = buildCanonicalCityPage("fuel-history", snap.state_code, cityName);
    const stateCanonical = buildCanonicalStatePage("fuel-history", snap.state_code);
    ...
    const bread = buildBreadcrumbLd([
        ...
        { name: "Fuel history", item: `${siteUrl}/fuel-history` },
