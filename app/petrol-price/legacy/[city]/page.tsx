...
export async function generateMetadata({
    ...
    if (resolved.kind !== "resolved") {
        const url = `${siteUrl}/petrol-price/${encodeURIComponent(params.city)}`;
        return {
    ...
    const canonical = `${siteUrl}/petrol-price/${encodeURIComponent(
...
export default async function PetrolPriceCityPage({
    params,
    searchParams,
}: {
    params: { city: string };
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                        {resolved.candidates.map((c) => (
                            <Link
                                ...
                                href={`/petrol-price/${encodeURIComponent(
    ...
    const snap = resolved.snapshot;
    permanentRedirect(
        `/petrol-price/${encodeURIComponent(
            snap.state_code.toLowerCase(),
    ...
    const bread = buildBreadcrumbLd([
        ...
        { name: `Petrol price in ${cityName}`, item: canonical },
    ]);
    ...
    return (
        <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-border/10 bg-card p-4">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-muted">Diesel price in {cityName} today</h2>
                <p className="mt-2 text-sm text-muted">
                    Diesel in {cityName} is {formatInr(todayRow.diesel_price) ?? "-"} per litre today.
                </p>
            </section>
            ...
            <section className="rounded-2xl border border-border/10 bg-card p-4">
                ...
                <p className="mt-2 text-sm text-muted">
                    Use our mileage-based calculator to estimate cost to travel 100 km in {cityName}.
                </p>
                <div className="mt-3">
                    <Link
